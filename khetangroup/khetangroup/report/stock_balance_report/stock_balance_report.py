...
class StockBalanceReport(object):
	...
	def prepare_item_warehouse_map(self, item_warehouse_map, entry, group_by_key):
		...
		qty_set_box = entry.set_box
		if entry.set_box:
			qty_dict_set = entry.set_box
		else:
			qty_dict_set = 1.0

		qty_dict.bal_con_qty = float(qty_dict.bal_qty) / float(qty_dict_set)
    ...
    {
					"label": _("Balance Converstion Qty"),
					"fieldname": "bal_con_qty",
					"fieldtype": "Float",
					"width": 100,
					"convertible": "qty",
				},
